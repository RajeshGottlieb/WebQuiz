...
public class XmlToDb {
    ...
    public XmlToDb(String filename) {
        ...
        try {
            ...
            Connection con = DriverManager.getConnection(sql_url, sql_user, sql_pass);
    ...
    void handleUser(Node node, Statement stmt) throws SQLException {
        ...
        int i = stmt.executeUpdate("INSERT INTO user (username, password) VALUES ('" + username + "', '" + password
                + "')");
        System.out.println("Inserted: user row OK using Statement.  " + i + " row(s)");
    }

    void handleSubject(Node node, Statement stmt) throws SQLException {
        String name = getAttribute(node, "name");

        int rc = stmt.executeUpdate("INSERT INTO subject (name) VALUES ('" + name + "')",
                Statement.RETURN_GENERATED_KEYS);
        System.out.println("Inserted: subject row OK using Statement.  " + rc + " row(s)");
    ...
    void handleCategory(Node node, int subject_id, Statement stmt) throws SQLException {
        String name = getAttribute(node, "name");

        int rc = stmt.executeUpdate("INSERT INTO category (subject_id, name) VALUES (" + subject_id + ", '" + name
                + "')", Statement.RETURN_GENERATED_KEYS);
        System.out.println("Inserted: category row OK using Statement.  " + rc + " row(s)");
    ...
    void handleModule(Node node, int category_id, Statement stmt) throws SQLException {
        String name = getAttribute(node, "name");

        int rc = stmt.executeUpdate("INSERT INTO module (category_id, name) VALUES (" + category_id + ", '" + name
                + "')", Statement.RETURN_GENERATED_KEYS);
        System.out.println("Inserted: module row OK using Statement.  " + rc + " row(s)");
        ResultSet rs = stmt.getGeneratedKeys();
    ...
    void handleQuestion(Node node, int module_id, Statement stmt) throws SQLException {
        ...
        int rc = stmt.executeUpdate("INSERT INTO question (module_id, type, text) VALUES (" + module_id + ", '" + type
                + "', '" + text + "')", Statement.RETURN_GENERATED_KEYS);
        System.out.println("Inserted: question row OK using Statement.  " + rc + " row(s)");
        ResultSet rs = stmt.getGeneratedKeys();
